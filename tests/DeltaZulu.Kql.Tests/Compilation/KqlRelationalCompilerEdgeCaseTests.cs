using DeltaZulu.Kql.Compilation;
using DeltaZulu.Kql.Relational;
using Microsoft.VisualStudio.TestTools.UnitTesting;

namespace DeltaZulu.Kql.Tests.Compilation;

/// <summary>
/// Edge case, error path, and adversarial tests for the KQL-to-RelNode compiler,
/// migrated from DeltaZulu.Platform's KustoToRelationalEdgeCaseTests. These verify
/// that the compiler rejects malformed, blocked, or adversarial KQL with
/// structured diagnostics rather than crashing or producing wrong trees.
/// </summary>
[TestClass]
public sealed class KqlRelationalCompilerEdgeCaseTests
{
    private static readonly KqlRelationalCompiler Compiler = new();

    private static KqlCompilationResult Translate(string kql) =>
        Compiler.Compile(kql, CanonicalTestCatalog.Instance);

    // ─── Broken / invalid KQL syntax ────────────────────────────────

    [TestMethod]
    [Description("Empty string produces diagnostic, not crash")]
    public void Syntax_EmptyString()
    {
        var result = Translate("");
        Assert.IsTrue(result.Root is null || result.HasErrors,
            "Empty KQL should produce null result or diagnostics");
    }

    [TestMethod]
    [Description("Whitespace-only string produces diagnostic")]
    public void Syntax_WhitespaceOnly()
    {
        var result = Translate("   \t\n  ");
        Assert.IsTrue(result.Root is null || result.HasErrors,
            "Whitespace-only KQL should produce null result or diagnostics");
    }

    [TestMethod]
    [Description("Completely invalid text produces parse error")]
    public void Syntax_Gibberish()
    {
        var result = Translate("!@#$%^&*()_+{}|:<>?");
        Assert.IsTrue(result.HasErrors, "Gibberish should produce parse errors");
        Assert.Contains(d => d.Phase == KqlDiagnosticPhase.Parse, result.Diagnostics,
            "Errors should be in Parse phase");
    }

    [TestMethod]
    [Description("Unterminated string literal produces parse error")]
    public void Syntax_UnterminatedString()
    {
        var result = Translate("""ProcessEvent | where FileName == "unterminated""");
        Assert.IsTrue(result.HasErrors, "Unterminated string should produce error");
    }

    [TestMethod]
    [Description("Missing pipe operator between clauses")]
    public void Syntax_MissingPipe()
    {
        var result = Translate(
            """ProcessEvent where FileName == "cmd.exe" """);
        Assert.IsTrue(result.HasErrors, "Missing pipe should produce error");
    }

    [TestMethod]
    [Description("Double pipe operator")]
    public void Syntax_DoublePipe()
    {
        var result = Translate("ProcessEvent || take 10");
        Assert.IsTrue(result.HasErrors, "Double pipe should produce error");
    }

    [TestMethod]
    [Description("Trailing pipe with no operator")]
    public void Syntax_TrailingPipe()
    {
        var result = Translate("ProcessEvent | take 10 |");
        Assert.IsTrue(result.HasErrors, "Trailing pipe should produce error");
    }

    [TestMethod]
    [Description("Leading pipe with no table")]
    public void Syntax_LeadingPipe()
    {
        var result = Translate("| take 10");
        Assert.IsTrue(result.HasErrors, "Leading pipe should produce error");
    }

    [TestMethod]
    [Description("Mismatched parentheses")]
    public void Syntax_MismatchedParens()
    {
        var result = Translate(
            """ProcessEvent | where (FileName == "cmd.exe" """);
        Assert.IsTrue(result.HasErrors, "Mismatched parens should produce error");
    }

    [TestMethod]
    [Description("take with non-numeric argument")]
    public void Syntax_TakeNonNumeric()
    {
        var result = Translate("""ProcessEvent | take "ten" """);
        Assert.IsTrue(result.HasErrors, "take with string should produce error");
    }

    [TestMethod]
    [Description("take with negative number")]
    public void Syntax_TakeNegative()
    {
        var result = Translate("ProcessEvent | take -5");
        Assert.IsTrue(result.HasErrors, "take with negative should produce error");
    }

    [TestMethod]
    [Description("extract() with too few args produces diagnostic")]
    public void Syntax_Extract_TooFewArguments()
    {
        var result = Translate(
            """
            ProcessEvent
            | extend bad = extract("-enc\\s+([^\\s]+)", 1)
            """);
        Assert.IsTrue(result.HasErrors, "extract with 2 args should fail");
        Assert.Contains(d => d.Message.Contains("extract", StringComparison.OrdinalIgnoreCase), result.Diagnostics,
            "Diagnostic should reference extract");
    }

    [TestMethod]
    [Description("extract() with too many args produces diagnostic")]
    public void Syntax_Extract_TooManyArguments()
    {
        var result = Translate(
            """
            ProcessEvent
            | extend bad = extract("-enc\\s+([^\\s]+)", 1, ProcessCommandLine, typeof(string), "extra")
            """);
        Assert.IsTrue(result.HasErrors, "extract with 5 args should fail");
    }

    [TestMethod]
    [Description("extract() with non-integer capture group produces semantic diagnostic")]
    public void Syntax_Extract_NonIntegerCaptureGroup()
    {
        var result = Translate(
            """
            ProcessEvent
            | extend bad = extract("-enc\\s+([^\\s]+)", "one", ProcessCommandLine)
            """);
        Assert.IsTrue(result.HasErrors, "extract requires integer capture group index");
    }

    // ─── Comments ───────────────────────────────────────────────────

    [TestMethod]
    [Description("Single-line comment before query")]
    public void Comment_SingleLineBefore()
    {
        var result = Translate(
            """
            // This is a hunting query
            ProcessEvent | take 10
            """);
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        Assert.IsNotNull(result.Root);
    }

    [TestMethod]
    [Description("Single-line comment after pipe")]
    public void Comment_SingleLineAfterPipe()
    {
        var result = Translate(
            """
            ProcessEvent
            | where FileName == "cmd.exe" // filter for cmd
            | take 10
            """);
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        Assert.IsNotNull(result.Root);
    }

    [TestMethod]
    [Description("Block comment inside expression")]
    public void Comment_BlockInExpression()
    {
        var result = Translate(
            """
            ProcessEvent
            | where /* only cmd */ FileName == "cmd.exe"
            | take 10
            """);
        Assert.IsTrue(result.HasErrors, string.Join("\n", result.Diagnostics));
    }

    [TestMethod]
    [Description("Comment-only input should not crash")]
    public void Comment_OnlyComment()
    {
        var result = Translate("// just a comment, nothing to run");
        // Should not crash; either null result or diagnostics
        Assert.IsTrue(result.Root is null || !result.HasErrors);
    }

    [TestMethod]
    [Description("Multi-line block comment spanning operators")]
    public void Comment_MultiLineBlock()
    {
        var result = Translate(
            """
            ProcessEvent
            /*
               This is a multi-line comment
               that spans several lines
            */
            | take 5
            """);
        Assert.IsTrue(result.HasErrors, string.Join("\n", result.Diagnostics));
    }

    // ─── Policy violations ──────────────────────────────────────────

    [TestMethod]
    [Description("Access to bronze.* table rejected")]
    public void Policy_RawTableAccess()
    {
        var result = Translate("bronze.windows_sysmon_event | take 10");
        Assert.IsTrue(result.HasErrors);
        Assert.Contains(d => d.Phase == KqlDiagnosticPhase.Policy ||
                                        d.Phase == KqlDiagnosticPhase.Parse, result.Diagnostics,
            "Raw table access should be rejected at policy or parse phase");
    }

    [TestMethod]
    [Description("Access to silver.* table rejected")]
    public void Policy_InternalTableAccess()
    {
        var result = Translate("silver.v_processevent_windows_sysmon_eid1 | take 10");
        Assert.IsTrue(result.HasErrors);
    }

    [TestMethod]
    [Description("Nonexistent table produces clear diagnostic")]
    public void Policy_NonexistentTable()
    {
        var result = Translate("FakeTable | take 10");
        Assert.IsTrue(result.HasErrors);
        Assert.Contains(d =>
            d.Message.Contains("approved") || d.Message.Contains("FakeTable"), result.Diagnostics,
            "Diagnostic should mention the table or approval");
    }

    [TestMethod]
    [Description("Schema-qualified approved table is allowed only for golden schema")]
    public void Policy_ApprovedTableWithDisallowedSchemaQualifier()
    {
        var result = Translate("silver.ProcessEvent | take 1");
        Assert.IsTrue(result.HasErrors);
        Assert.Contains(
            d => d.Phase == KqlDiagnosticPhase.Policy &&
                 d.Message.Contains("Table path", StringComparison.OrdinalIgnoreCase),
            result.Diagnostics,
            "Expected policy rejection for disallowed schema-qualified table path.");
    }

    [TestMethod]
    [Description("Fully-qualified approved table is allowed only for hunting.golden")]
    public void Policy_ApprovedTableWithDisallowedDatabaseQualifier()
    {
        var result = Translate("otherdb.golden.ProcessEvent | take 1");
        Assert.IsTrue(result.HasErrors);
        Assert.Contains(
            d => d.Phase == KqlDiagnosticPhase.Policy &&
                 d.Message.Contains("Table path", StringComparison.OrdinalIgnoreCase),
            result.Diagnostics,
            "Expected policy rejection for disallowed database-qualified table path.");
    }

    [TestMethod]
    [Description("Golden-qualified approved table is rejected with explicit policy diagnostic")]
    public void Policy_ApprovedTableWithGoldenQualifierRejected()
    {
        var result = Translate("golden.ProcessEvent | take 1");
        Assert.IsTrue(result.HasErrors);
        Assert.Contains(
            d => d.Phase == KqlDiagnosticPhase.Policy &&
                 d.Message.Contains("Table path", StringComparison.OrdinalIgnoreCase),
            result.Diagnostics,
            "Expected policy rejection for qualified approved table path.");
    }

    [TestMethod]
    [Description("Bare join (no kind=) blocked with clear message")]
    public void Policy_BareJoinBlocked()
    {
        var result = Translate(
            "ProcessEvent | join (ProcessEvent | take 5) on DeviceName");
        Assert.IsTrue(result.HasErrors);
        Assert.IsNull(result.Root);
        var joinDiag = result.Diagnostics.FirstOrDefault(d =>
            d.Message.Contains("innerunique") || d.Message.Contains("kind"));
        Assert.IsNotNull(joinDiag, "Diagnostic should explain innerunique semantics");
    }

    [TestMethod]
    [Description("innerunique join explicitly blocked")]
    public void Policy_InneruniqueJoinBlocked()
    {
        var result = Translate(
            "ProcessEvent | join kind=innerunique (ProcessEvent | take 5) on DeviceName");
        Assert.IsTrue(result.HasErrors);
    }

    // ─── Management commands rejected ───────────────────────────────

    [TestMethod]
    [Description("Dot-command .show rejected")]
    public void Policy_DotCommandShow()
    {
        var result = Translate(".show tables");
        Assert.IsTrue(result.HasErrors || result.Root is null,
            ".show command should be rejected");
    }

    [TestMethod]
    [Description("Dot-command .drop rejected")]
    public void Policy_DotCommandDrop()
    {
        var result = Translate(".drop table ProcessEvent");
        Assert.IsTrue(result.HasErrors || result.Root is null,
            ".drop command should be rejected");
    }

    [TestMethod]
    [Description("Dot-command .set rejected")]
    public void Policy_DotCommandSet()
    {
        var result = Translate(".set MyTable <| ProcessEvent | take 10");
        Assert.IsTrue(result.HasErrors || result.Root is null,
            ".set command should be rejected");
    }

    [TestMethod]
    [Description("Query followed by management command is rejected as multi-statement input")]
    public void Policy_QueryThenDotCommandRejected()
    {
        var result = Translate(
            """
            ProcessEvent | where FileName == "cmd.exe";
            .drop table ProcessEvent
            """);

        Assert.IsNull(result.Root, "Translator must reject multi-statement input that chains commands.");
        Assert.IsTrue(result.HasErrors, "A diagnostic should be raised for mixed query/command input.");
        Assert.Contains(
            d =>
                d.Message.Contains("Management commands are not allowed. Submit only a single query expression, optionally preceded by let bindings."), result.Diagnostics,
            "Expected rejection diagnostic for mixed query/command input was not emitted.");
    }

    [TestMethod]
    [Description("Ordinary quoted string spanning newline should not be translated as a recovered partial query")]
    public void Policy_OrdinaryQuotedStringWithNewline_FailsClosed()
    {
        var result = Translate(
            """
        let Suspicious = ";
        .drop table ProcessEvent";
        ProcessEvent
        | where ProcessCommandLine contains Suspicious
        | take 1
        """);

        Assert.IsNull(result.Root);
        Assert.Contains(d =>
            d.Phase == KqlDiagnosticPhase.Parse ||
            d.Phase == KqlDiagnosticPhase.Policy, result.Diagnostics);
    }

    [TestMethod]
    [Description("SQL-like payload inside KQL string literal does not become a second statement")]
    public void Parse_StringLiteralSqlPayload_RemainsSingleStatement()
    {
        var result = Translate(
            """ProcessEvent | where FileName == "'; DROP TABLE golden.ProcessEvent;--" | take 1""");

        Assert.IsNotNull(result.Root, "Payload embedded in a KQL string should remain data, not a statement separator.");
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics.Select(d => d.Message)));
    }

    // ─── Type and column errors ─────────────────────────────────────

    [TestMethod]
    [Description("Reference to nonexistent column in approved table")]
    public void Column_Nonexistent()
    {
        var result = Translate(
            """ProcessEvent | where FakeColumn == "test" """);
        // Kusto.Language semantic analysis should catch this
        Assert.IsTrue(result.HasErrors,
            "Reference to nonexistent column should produce error");
    }

    [TestMethod]
    [Description("Type mismatch in comparison (string == int) caught by analyzer")]
    public void Column_TypeMismatch()
    {
        var result = Translate(
            "ProcessEvent | where FileName == 42");
        // Kusto.Language may or may not flag this — depends on implicit conversion
        // At minimum it should not crash
        Assert.IsNotNull(result, "Translation should complete without crash");
    }

    // ─── Operator edge cases ────────────────────────────────────────

    [TestMethod]
    [Description("where with always-true predicate")]
    public void Operator_WhereAlwaysTrue()
    {
        var result = Translate("ProcessEvent | where true | take 5");
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        Assert.IsNotNull(result.Root);
    }

    [TestMethod]
    [Description("where with always-false predicate")]
    public void Operator_WhereAlwaysFalse()
    {
        var result = Translate("ProcessEvent | where false | take 5");
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        Assert.IsNotNull(result.Root);
    }

    [TestMethod]
    [Description("Multiple where clauses in sequence")]
    public void Operator_MultipleWhere()
    {
        var result = Translate(
            """
            ProcessEvent
            | where FileName == "cmd.exe"
            | where ProcessId > 100
            | where DeviceName != ""
            | take 10
            """);
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        // Should produce nested FilterNodes
        var limit = result.Root as LimitNode;
        Assert.IsNotNull(limit);
    }

    [TestMethod]
    [Description("project with zero columns should error or produce valid SQL")]
    public void Operator_ProjectEmpty()
    {
        var result = Translate("ProcessEvent | project");
        // Kusto parser should reject this
        Assert.IsTrue(result.HasErrors, "project with no columns should error");
    }

    [TestMethod]
    [Description("summarize with no aggregation function")]
    public void Operator_SummarizeNoAgg()
    {
        var result = Translate("ProcessEvent | summarize by FileName");
        // Valid KQL — equivalent to "distinct FileName" in Kusto
        Assert.IsNotNull(result, "Should not crash");
    }

    [TestMethod]
    [Description("sort by multiple columns")]
    public void Operator_SortMultiple()
    {
        var result = Translate(
            "ProcessEvent | sort by DeviceName asc, Timestamp desc | take 10");
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        Assert.IsNotNull(result.Root);
    }

    [TestMethod]
    [Description("top with multiple sort columns")]
    public void Operator_TopMultiSort()
    {
        var result = Translate(
            "ProcessEvent | top 5 by Timestamp desc");
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        var limit = result.Root as LimitNode;
        Assert.IsNotNull(limit);
        Assert.AreEqual(5, limit!.Count);
    }

    [TestMethod]
    [Description("Trivial function batch invalid arity is rejected with diagnostics")]
    public void Functions_TrivialBatch_InvalidArity()
    {
        var result = Translate(
            """
            ProcessEvent
            | extend
                A = strcat_array(Tags),
                B = bag_keys(AdditionalFields, "extra"),
                C = bag_has_key(AdditionalFields),
                D = bag_merge(AdditionalFields),
                E = array_length(),
                F = exp2(),
                G = exp10(1, 2),
                H = hash_sha256(FileName, "extra"),
                I = hash_md5(),
                J = translate("abc", FileName)
            """);

        Assert.IsTrue(result.HasErrors, "invalid function arity should produce diagnostics");
    }

    [TestMethod]
    [Description("hash_sha256, hash_md5, and translate invalid arity is rejected with diagnostics")]
    public void Functions_HashAndTranslate_InvalidArity()
    {
        string[] invalidCalls =
        [
            "hash_sha256(FileName, 'extra')",
            "hash_md5()",
            "translate('abc', FileName)"
        ];

        foreach (var invalidCall in invalidCalls)
        {
            var result = Translate($"ProcessEvent | extend Invalid = {invalidCall}");
            Assert.IsTrue(result.HasErrors, $"{invalidCall} should produce an invalid-arity diagnostic");
        }
    }

    [TestMethod]
    [Description("hash_sha256 and hash_md5 reject non-string scalars until KQL scalar serialization is implemented")]
    public void Functions_Hash_NonStringInput_Rejected()
    {
        string[] unsupportedCalls =
        [
            "hash_sha256(ProcessId)",
            "hash_md5(datetime(2020-01-01))"
        ];

        foreach (var unsupportedCall in unsupportedCalls)
        {
            var result = Translate($"ProcessEvent | extend Invalid = {unsupportedCall}");
            Assert.Contains(
                diagnostic => diagnostic.Message.Contains("only string input", StringComparison.OrdinalIgnoreCase),
                result.Diagnostics,
                $"{unsupportedCall} should produce a string-only hash diagnostic");
        }
    }

    // ─── Whitespace and formatting edge cases ───────────────────────

    [TestMethod]
    [Description("Excessive whitespace between operators")]
    public void Whitespace_Excessive()
    {
        var result = Translate(
            "ProcessEvent    |    take     10");
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        Assert.IsNotNull(result.Root);
    }

    [TestMethod]
    [Description("Tab-indented KQL")]
    public void Whitespace_TabIndented()
    {
        var result = Translate(
            "ProcessEvent\n\t| where FileName == \"cmd.exe\"\n\t| take 10");
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        Assert.IsNotNull(result.Root);
    }

    [TestMethod]
    [Description("Windows-style line endings (CRLF)")]
    public void Whitespace_Crlf()
    {
        var result = Translate(
            "ProcessEvent\r\n| where FileName == \"cmd.exe\"\r\n| take 10");
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        Assert.IsNotNull(result.Root);
    }

    [TestMethod]
    [Description("Single-line compact KQL")]
    public void Whitespace_Compact()
    {
        var result = Translate(
            """ProcessEvent|where FileName=="cmd.exe"|take 10""");
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        Assert.IsNotNull(result.Root);
    }

    // ─── Adversarial and stress inputs ──────────────────────────────

    [TestMethod]
    [Description("Very long column name does not crash")]
    public void Adversarial_LongColumnName()
    {
        var longName = new string('A', 500);
        var result = Translate(
            $"ProcessEvent | where {longName} == \"test\"");
        // Should produce error (column doesn't exist) but not crash
        Assert.IsTrue(result.HasErrors || result.Root is not null);
    }

    [TestMethod]
    [Description("Very long string literal does not crash")]
    public void Adversarial_LongStringLiteral()
    {
        var longValue = new string('X', 10_000);
        var result = Translate(
            $"ProcessEvent | where FileName == \"{longValue}\" | take 1");
        Assert.IsNotNull(result, "Should not crash on long string");
    }

    [TestMethod]
    [Description("Many chained where clauses do not stack overflow")]
    public void Adversarial_ManyWheres()
    {
        var kql = "ProcessEvent\n";
        for (var i = 0; i < 50; i++)
        {
            kql += $"| where ProcessId > {i}\n";
        }

        kql += "| take 5";

        var result = Translate(kql);
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        Assert.IsNotNull(result.Root);
    }

    [TestMethod]
    [Description("Many chained extend clauses do not stack overflow")]
    public void Adversarial_ManyExtends()
    {
        var kql = "ProcessEvent\n";
        for (var i = 0; i < 30; i++)
        {
            kql += $"| extend col_{i} = ProcessId + {i}\n";
        }

        kql += "| take 5";

        var result = Translate(kql);
        Assert.IsFalse(result.HasErrors, string.Join("\n", result.Diagnostics));
        Assert.IsNotNull(result.Root);
    }

    // ─── Diagnostic quality ─────────────────────────────────────────

    [TestMethod]
    [Description("All diagnostics have non-empty messages")]
    public void DiagnosticQuality_NonEmptyMessages()
    {
        // Run several failing translations and check diagnostic quality.
        // The original Platform test also asserted that no diagnostic carries an
        // Emit or Execute phase; KqlDiagnosticPhase has no such members at all
        // (only Parse/Policy/Translate -- what compilation itself produces), so
        // that guarantee is now enforced by the type system rather than a check.
        string[] badInputs =
        [
            "FakeTable | take 10",
            "!@#$",
            "",
            "ProcessEvent | join (ProcessEvent) on DeviceName",
        ];

        foreach (var input in badInputs)
        {
            var result = Translate(input);
            foreach (var d in result.Diagnostics)
            {
                Assert.IsFalse(string.IsNullOrWhiteSpace(d.Message),
                    $"Diagnostic for input '{input}' has empty message");
            }
        }
    }

    [TestMethod]
    [Description("Diagnostics carry correct phase identifiers")]
    public void DiagnosticQuality_CorrectPhase()
    {
        // Parse error
        var result1 = Translate("!!! broken syntax !!!");
        if (result1.HasErrors)
        {
            Assert.IsTrue(result1.Diagnostics.All(d => d.Phase == KqlDiagnosticPhase.Parse ||
                                          d.Phase == KqlDiagnosticPhase.Translate));
        }

        // Policy error
        var result2 = Translate("FakeTable | take 10");
        if (result2.HasErrors)
        {
            Assert.Contains(d => d.Phase == KqlDiagnosticPhase.Policy ||
                                          d.Phase == KqlDiagnosticPhase.Parse, result2.Diagnostics);
        }
    }
}
